...
router.put('/', (req, res) => {
  ...
  if (exists && fs.statSync(filePath).isDirectory())
    return res.status(400).send('Cannot write to directory');
  if (exists) {
...
router.post('/', async (req, res) => {
  ...
  await file
    ...
    .then(() => {
      ...
      res
        .status(201)
        .send({ created: true, files: readData(req.homeDirectory) });
    })
    .catch((err) => {
      console.log(err);
      res.sendStatus(500);
...
router.put('/description', async (req, res) => {
  ...
  if (!oldFile) return res.sendStatus(404);

  await File.findOneAndUpdate({ location: filePath }, updatedFile)
    .then(() => {
      res
        .status(200)
        .send({ updatedDescription: true, files: readData(req.homeDirectory) });
    })
    .catch((err) => {
      console.log(err);
      res.sendStatus(500);
...
router.delete('/', (req, res) => {
  ...
  if (!fs.existsSync(filePath)) return res.status(404).send('File not found');
  if (fs.statSync(filePath).isDirectory())
    return res.status(400).send('Cannot delete directory');
