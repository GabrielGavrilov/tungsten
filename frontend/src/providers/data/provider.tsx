...
type DataContextProps = {
  ...
  editDescription: (path: string, description: string) => Promise<DataLeaf>;
...
export function DataProvider({
  ...
  const updateFiles = useCallback((files: DataLeaf | null) => {
    if (files) {
      ...
      const deletedLeaf = files.children.find(
        (child) => child.name === '.trash'
      );
      if (deletedLeaf?.type === 'directory') {
        setDeleted(deletedLeaf.children.map((child) => child.name));
      }
      files.children = files.children.filter(
        (child) => child.name !== '.trash'
      );
      setFiles(files);
    } else {
  ...
  const deleteFile = useCallback(
    async (path: string) => {
      const response = await fetch(
        withQueryParams(endpoints.files.index, { filePath: path }),
        {
          method: 'DELETE',
          credentials: 'include',
        }
  ...
  const deleteDirectory = useCallback(
    async (path: string) => {
      const response = await fetch(
        withQueryParams(endpoints.folders.index, { folderPath: path }),
        {
          method: 'DELETE',
          credentials: 'include',
        }
  ...
  const renameFile = useCallback(
    async (path: string, newPath: string) => {
      const response = await fetch(
        withQueryParams(endpoints.files.name, { oldPath: path, newPath }),
        {
          method: 'PUT',
          credentials: 'include',
        }
  ...
  const editDescription = useCallback(
    async (path: string, description: string) => {
      const response = await fetch(
        withQueryParams(endpoints.files.description, {
          filePath: path,
  ...
  const renameDirectory = useCallback(
    async (path: string, newPath: string) => {
      const response = await fetch(
        withQueryParams(endpoints.folders.name, { oldPath: path, newPath }),
        {
          method: 'PUT',
          credentials: 'include',
        }
  ...
  const restoreFile = useCallback(
    async (name: string) => {
      const response = await fetch(
        withQueryParams(endpoints.deleted.restore, { filePath: name }),
        {
          method: 'PUT',
          credentials: 'include',
        }
  ...
  const permanentlyDeleteFile = useCallback(
    async (name: string) => {
      const response = await fetch(
        withQueryParams(endpoints.deleted.index, { filePath: name }),
        {
          method: 'DELETE',
          credentials: 'include',
        }
